...
Edit the parameters below and run. The script also writes them into the
design as user parameters so they are visible in Modify > Change Parameters,
but the geometry is generated by this script: change a value here and re-run
rather than editing the parameter in the UI.
...
def write_user_parameters(des):
    """Mirror the inputs into the design so they are visible in the UI."""
    units = des.unitsManager
    wanted = [
    ...
    for name, value, unit, comment in wanted:
        existing = des.userParameters.itemByName(name)
        if existing:
            existing.expression = ('%g %s' % (value, unit)).strip()
            continue
        des.userParameters.add(
            name,
            adsk.core.ValueInput.createByString(('%g %s' % (value, unit)).strip()),
            unit, comment)
    return units
...
def run(_context: str):
    ...
    body.name = 'Gridfinity Spool Holder %dx%dx%du' % (GRID_X, GRID_Y, GRID_Z)
    bb = body.boundingBox
    print('pegs: %d' % n)
